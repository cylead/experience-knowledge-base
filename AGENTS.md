# Reading Knowledge Vault Instructions

These instructions apply to the entire vault. Markdown files and their YAML properties are the source of truth.

## Purpose

Help the user turn reading reflections into a durable, connected system of books, atomic insights, defined concepts, practices, and weekly reviews. Preserve the user's meaning and provenance. Do not treat model-generated content as if the user said it.

## Privacy boundary

- Work only with the user's reflections, relevant vault notes, metadata, and short excerpts.
- Never request, import, or process a complete copyrighted book.
...
The approval gate applies whenever the user asks to capture or record a reading reflection. It does not apply to explicit requests to maintain the vault's templates, schema, documentation, or views.

1. Inspect existing books, insight titles, concept names, aliases, definitions, and practices.
2. Present a proposed change set in chat. Do not create or edit vault files at this stage.
3. Include:
   - the verbatim reading-log entry and target book;
...
- Append raw input verbatim to the matching book's `## Reading Log` as a block quote.
- Use a timestamped heading and a stable block identifier: `^capture-YYYYMMDD-book-slug-NN`.
...
- Compare the exact raw input with the target book's reading log. Do not append an identical capture twice.
...
- Book filename: concise canonical title, such as `Stolen Focus.md`.
...
- Allowed practice status values: `proposed`, `active`, `paused`, `retired`.

## Weekly review workflow

When the user says “Start my weekly review”:

1. Read active practices whose `next_review` is today or earlier, plus the most recent weekly review.
